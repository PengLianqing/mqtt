...
def subscribe(client: mqtt_client): #订阅消息
    def on_message(client, userdata, msg):
        ...
        revc_data = json.loads(json_str) #对数据进行解码,将JSON对象解码为Python字典
        try:
            print(f"Received `{revc_data['messages']}` ,`{revc_data['userDate']}` ,`{revc_data['userStr']}` from `{msg.topic}` topic")
        except KeyError: #查找字典出错
            print("KeyError :dict key error.")
        else:
            pass
         
    client.subscribe(topic) #订阅目标消息
    client.subscribe("python/status") #订阅发送端的遗嘱消息
